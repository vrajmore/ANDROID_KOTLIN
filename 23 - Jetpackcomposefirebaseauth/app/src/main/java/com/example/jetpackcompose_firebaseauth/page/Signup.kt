...
@Composable
fun Signup(firebaseauthviewmodel: firebaseauthviewmodel, navController: NavController) {
...
//            is AuthState.error -> Toast.makeText(context, "${(authstate.value as AuthState.error).message}", Toast.LENGTH_SHORT).show()
    ...
    Column(
        ...
        Button(
            onClick = {
                firebaseauthviewmodel.signup(Email, Password, context)
            }, enabled = authstate.value != AuthState.loading
        ) {
            Text(text = "Create Account")
        }
        TextButton(onClick = { navController.navigate("Login") }) { Text(text = "Already have account? Click to Login") }
    }
}
